...
import com.sn.go4lunch.Fragment.MapFragment;
...
import com.sn.go4lunch.View.DetailAdapter;
...
import static com.sn.go4lunch.View.RestaurantViewHolder.BASE_URL;
import static com.sn.go4lunch.View.RestaurantViewHolder.MAX_HEIGHT_LARGE;
import static com.sn.go4lunch.View.RestaurantViewHolder.MAX_RATING;
import static com.sn.go4lunch.View.RestaurantViewHolder.MAX_STAR;
...
public class PlaceDetailActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void executeHttpRequestWithRetrofit(String placeId) {
        this.mDisposable = PlacesStream.streamSimpleFetchPlaceInfo(placeId, MapFragment.API_KEY).subscribeWith(createObserver());
    }
    ...
    private void updateUI(PlaceDetailsInfo results) {
        if (results != null) {
            if (getCurrentUser() != null) {
                this.checkIfUserAlreadyBookedRestaurant(getCurrentUser()
                        .getUid(), requestResult.getPlaceId(), requestResult.getName(), false);
                ...
                Toast.makeText(this,
                        getResources().getString(R.string.restaurant_error_retrieving_info), Toast.LENGTH_SHORT).show();
            }
                ...
                if (restaurantTask.getResult().size() == 1) { // User already booked a restaurant today
                    ...
                    for (QueryDocumentSnapshot restaurant : restaurantTask.getResult()) {
                        if (restaurant.getData().get("restaurantName").equals(restaurantName)) {
                            // If booked restaurant is the same as restaurant we are trying to book
                            this.displayFAB((R.drawable.baseline_clear_black_24), getResources().getColor(R.color.colorError));
                            if (tryingToBook) {
                                this.manageBooking(
                                        userId, restaurantId, restaurantName, restaurant.getId()
                                        , false, false, true);
                                Toast.makeText(this, getResources()
                                        .getString(R.string.restaurant_cancel_booking), Toast.LENGTH_SHORT).show();
                            }

                        } else {
                            // If user is trying to book an other restaurant for today
                            this.displayFAB((R.drawable.baseline_check_circle_black_24), getResources().getColor(R.color.colorGreen));
                            if (tryingToBook) {
                                this.manageBooking(
                                        userId, restaurantId, restaurantName, restaurant.getId(),
                                        false, true, false);
                                Toast.makeText(this, getResources()
                                        .getString(R.string.restaurant_change_booking), Toast.LENGTH_SHORT).show();
                            }
                        }
                    }
    ...
    private void manageBooking(String userId, String restaurantId, String restaurantName,
                               @Nullable String bookingId, boolean toCreate, boolean toUpdate, boolean toDelete) {
        if (toUpdate) {
            RestaurantsHelper.deleteBooking(bookingId);
            RestaurantsHelper.createBooking(
                    this.getTodayDate(), userId, restaurantId, restaurantName)
