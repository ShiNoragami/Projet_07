package com.sn.go4lunch.Utils.Notification;
...
public class AlarmReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (FirebaseAuth.getInstance().getCurrentUser() != null) {
            RestaurantsHelper.getBooking(FirebaseAuth.getInstance()
                            .getCurrentUser().getUid(),
                    getTodayDate()).addOnCompleteListener(restaurantTask -> {
                if (restaurantTask.isSuccessful()) {
                    if (!(restaurantTask.getResult().isEmpty())) { // User did a booking for today
                        ...
                        for (DocumentSnapshot restaurant : restaurantTask.getResult()) {
                            RestaurantsHelper.getTodayBooking(restaurant.getData()
                                            .get("restaurantId").toString(),
                                    getTodayDate()).addOnCompleteListener(bookingTask -> {
                                if (bookingTask.isSuccessful()) {
                                    for (QueryDocumentSnapshot booking : bookingTask.getResult()) {
                                        UserHelper.getUser(booking.getData()
                                                .get("userId").toString())
                                                .addOnCompleteListener(userTask -> {
                                                    if (userTask.isSuccessful()) {
                                                        if (!(userTask.getResult().getData()
                                                                .get("uid").toString().equals(FirebaseAuth
                                                                        .getInstance().getCurrentUser().getUid()))) {
                                                            Log.e("TAG", "ALARM_RECEIVER | User : "
                                                                    + userTask.getResult().getData().get("username"));
                                                            String username = userTask.getResult().getData().get("username").toString();
                                                            usersList.add(username);
                                                        }
                                                    }
                                                    if (usersList.size() == bookingTask.getResult().size() - 1) {
                                                        this.executeHttpRequestWithRetrofit(restaurant
                                                                .getData().get("restaurantId").toString());
                                                    }
                                                });
                                    }
    ...
    public void sendNotification(String users) {
        ...
        PendingIntent pendingIntent = PendingIntent
                .getActivity(mContext, NotificationHelper.ALARM_TYPE_RTC,
                        resultIntent,
